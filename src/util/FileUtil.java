...
public class FileUtil {
	...
	public static MultipartRequest upload(
		...
		*/
		try {
			mr = new MultipartRequest(req, saveDirectory, 1024*1024, "UTF-8", 
	...
	/*
	파일 다운로드 처리2 : 원본파일명과 저장된 파일명을 지정하여 다운로드 시
		파일명을 원본파일명으로 변경한다.
	...
	*/
	public static void download(
		...
		*/
		...
		try{
			//파일명이 한글인 경우에는 깨짐현상이 발생하므로 반드시 인코딩 처리를 해줘야 한다.
			...
			*/
			...
			/*
			서버에 저장된 파일명을 통해 파일객체를 생성한 후 파일을 다운로드 받을 때
			아래 설정된 응답헤더를 통해 파일명은 원본파일명으로 변경되어 다운로드된다.
			*/
			response.setHeader("Content-Disposition"
