...
<body>
	...
	*/
	...
	<!--  
		최초 실행 시에는 파라미터가 없으므로 0으로 간주되어 계산된다.
		만약
			해당페이지?myNumber=20 		=> 결과 30 출력
			해당페이지?myNumber=		=> 0으로 간주되어 10출력
			해당페이지?myNumber=Three	=> 문자는 숫자로 변경 불가능[에러]
	-->
	...
	<!--  
		...
		비교연산은 null과의 비교 자체가 불가능하기 때문이다.
	-->
	...
	JSP에서 변수를 EL에서 사용할 수 없는 이유는 EL은 4가지 영역에
	저장된 속성들만 사용하기 때문이다. JSTL도 동일한 특성을 가지고 있다.
	*/
	...
	<!--  
		...
		하지만 개발 시에는 실제 서비스할 웹서버의 버전을 확인한 후
		사용여부를 결정해야 한다. EL은 전통적으로 값을 표현(출력)하는 용도로
		사용되어졌으므로 표현용으로만 사용하는 것이 좋다.
	-->
	<h3>EL변수에 값 할당</h3>
	...
	<!--  
		...
		나눗셈을 위한 / 연산자 대신 div, 나머지를 구하는 % 대신
		mod를 사용할 수 있다.
	-->
	<h3>EL의 산술연산자</h3>
	<ul>
		...
		<!--  
			...
			아래 문장 중 "100"은 자동으로 숫자로 변경된 후 연산된다.
	...
	<!--  
		EL에서는 비교연산자를 이용한 비교 시 변수의 값을 모두 문자열로
		인식하여 String클래스의 compareTo()와 같은 방식으로 비교한다.
		즉, 첫 번째 문자부터 하나씩 비교해 나간다.
		단 실제 숫자의 비교 시에는 일반적인 숫자비교가 이루어진다.
	-->
	<h3>EL의 비교연산자</h3>
	...
	<ul>
		<!-- 
			...
			저장된다. 따라서 비교 시 객체상태에서 비교가 이루어지게 된다.
	...
	<ul>
		...
		<li>\${not empty elemptyStr } : ${empty elemptyStr }</li>
		<li>\${empty ellengthZero ?
				"배열크기가0임" : "배열크기가0이아님"}</li>
		<li>\${not empty elsizeZero ?
				"컬렉션에 저장된 객체있음" : "컬렉션에 저장된 객체있음"}</li>
	</ul>
</body>
</html>