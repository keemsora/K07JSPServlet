...
<fieldset>
   ...
   <!-- 
      문자열 형태로 숫자가 전달되면 연산 시 자동으로 숫자가 변경된 후 
      연산된다. 숫자로 변경할 수 없는 문자가 포함된다면 예외가 발생한다.
      또한 문자열끼리의 연결도 EL에서는 허용되지 않는다.
   -->
   <h2>+를 숫자가 아닌 문자열 연결에 사용 시(에러)</h2>
   <ul>
      <li>\${"100"+10 } : ${"100"+10 } [정상]</li>
      <li>\${"백"+10 } : ${"100"+10 } [에러]</li>
      <li>\${"HELLO"+"EL" } : \${'HELLO'+'EL' } [에러]</li>
      <li>${"HELLO"} ${"EL" } <!-- 문자열은 이와 같이 표현하면 된다. --></li>
   </ul>
   
   <!--  
      EL에서 속성에 접근 시 -이 포함된 경우에는 .으로 표현할 수 없고
      반드시 [를 통해 표현해야 한다.
   -->
   <h2>특수문자가 포함된 속성에 .으로 접근 시</h2>
   <ul>
   ...
   <ul>
      ...
      <!-- MemberDTO에 getId()를 주석처리한 후 테스트 해 볼 것 -->
      <li>속성은존재, Getter가없는경우 : \${eldto.id }
         :
         \${eldto.id} [500에러발생]
      </li>
      <li>el변수명이틀린경우 : \${noeldto.pwd }
         :
   ...
   <ul>
      <li>\${requestScope.nop } : ${requestScope.nop }
         [Scope계열 내장객체, 에러NO, 미출력
            -> 속성명은 개발자 마음대로 정할 수 있기 때문]</li>
      <!--  
      	EL에서는 영역에 접근할 때 xxScope계열의 내장객체를 사용한다.
      	pageContext는 JSP에서 사용하는 내장객체이므로 에러가 발생한다.
      -->
      <li>\${pageContext.nop } : \${pageContext.nop }
         [컴파일 오류 발생 -> 내장객체명은 정해져 있기 때문]</li>
   </ul>
   ...
   <h2>EL변수에 값 할당 시 [톰캣8.0부터는 정상, 이전버전은 에러]</h2>
