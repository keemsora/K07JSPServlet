...
<body>
	<%--
	웹어플리케이션을 구성하는 모든 JSP(Servlet)에서 String형의 상수를
	공유할 목적으로 사용한다. web.xml에 <context-param>엘리먼트를 통해
	등록하게 된다. 이를 컨텍스트 초기화 파라미터라고 한다.
	--%>
	...
	<%--
	ServletContext의 메소드인 getRealPath()로 서버의 물리적경로를
	얻어올 수 있다. 사용 시 context root를 제외한 /로 시작하는 절대경로를
	입력해야 한다.
	--%>
	...
	<%--
	선언부(자바영역)에서 JSP의 내장객체를 사용할 때는 
	해당 내장객체의 클래스타입으로 사용해야 한다. 
	application객체는 ServletContext타입을 가지고 있다.
	--%>	
	...
	//2.매개변수로 내장객체를 전달하기
	/*
	Java영역에서는 JSP의 내장객체를 바로 사용하는 것은 불가능한다.
	반드시 매개변수 형태로 전달한 후 내장객체의 클래스형으로 받은 다음
	사용해야 한다.
	*/
