...
<body>
	<!-- 
		어플리케이션 영역에 저장된 속성은 페이지이동, 포워드되더라도
		그 영역이 공유되고 웹브라우저를 종료했다가 다시 실행하더라도
		계속 공유된다. 즉 한번 저장되면 웹서버가 셧다운(종료)될 때까지는
		속성값이 유지된다.
	-->
	...
	<!-- 
		...
		실행하게 되면 Nullpointer예외가 발생하게 된다.
		아래 코드는 application영역에 저장된 속성이 있다는 가정 하에
		속성을 읽어오게 되는데 저장된 속성이 없기 때문에 오류가 발생한다.
	-->
	...
	<h4>해당페이지에서만 사용가능하고, 페이지 이동 또는 forward되면
	해당 영역은 사라진다.</h4>
	<%
	...
	MemberDTO requestMember =
		(MemberDTO)request.getAttribute("pageMember");
	String rMemberStr =
			String.format("아이디:%s, 이름:%s, 비번:%s, 등록일:%s",
