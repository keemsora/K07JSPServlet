...
<body>
	...
	<ul>
		...
		<li>MemberDTO타입 : 아이디:<%=member.getId() %>,
							비번:<%=member.getPass() %>,
							이름:<%=member.getName() %>,
	...
	<%
		request.removeAttribute("NoKeyValue");//키 없으나 에러발생 안 됨
	...
	URL주소창에는 최초 요청된 페이지의 경로명이 보여지지만, 출력되는 페이지에는
	새롭게 전달된 페이지의 내용을 출력한다.
	...
	<%--
	sendRedirect()는 JS의 location.href와 동일한 기능으로
	새로운 페이지에 대한 요청을 하게 된다. 이 경우 request영역은 
	공유되지 않는다. 웹브라우저의 주소창에는 새롭게 요청된 페이지의 경로명이 
	보여지게 된다.
	--%>
