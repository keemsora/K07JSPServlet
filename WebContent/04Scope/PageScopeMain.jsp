...
<%
/*
JSP의 모든 영역(페이지, 리퀘스트, 세션, 어플리케이션)에는
모든 타입의 객체를 저장할 수 있다. Object타입으로 자동 형변환되어 저장되고,
사용 시에는 각 객체로 형변환 후 사용하면 된다.
차후 EL과 JSTL에서도 주로 해당 영역을 사용하게 된다.

-page영역
	페이지 영역에 저정된 속성은 해당 페이지에서만 공유되고,
	페이지를 벗어나는 순간 소멸된다.
*/
//페이지 영역에 데이터를 저장
pageContext.setAttribute("pageNumber", 1000);//Integer객체(Wrqpper클래스)
...
<body>
	...
	/*
	page영역에 저장된 속성을 읽어옴 
	영역에 저장될 때는 Object형으로 자동형변환되므로 
	읽어올 때도 Object형으로 가져오게 된다.
	*/
	Object obj = pageContext.getAttribute("pageDate");
	String dateString = "";
	//해당 객체가 Date형으로 형변환이 가능하다면
	...
	*/
	...
	<%--
		페이지영역에 저장된 속성값은 새로운 페이지에 대한 요청이 발생되면
		속성값이 모두 파괴되어 소멸된다.
	...
	<br />
	<%--
		외부파일을 인클루드 하는 것은 해당 파일을 원본 그대로
		현재 문서에 포함시킨 후 컴파일되므로 include지시어를 사용하는 경우
