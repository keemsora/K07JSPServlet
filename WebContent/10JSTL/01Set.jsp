...
<body>
	...
	set태그속성
		var : 변수명으로 문자열만 가능
		value : 값을 설정, 표현식/EL식 모두 가능
		scope : 영역을 지정하여 변수를 저장
	...
	<h3>EL로 출력</h3>
	<!--  
		EL로 영역의 속성을 읽어올 때는 xxScope를 생략하면
		가장 좁은 영역인 page영역을 읽어오게되므로 
		속성명이 동일하지 않다면 내장객체는 생략할 수 있다.
	-->
	<ul>
	...
	<h3>JSP코드로 출력</h3>
	<!-- 영역에 저장될 때 Object타입으로 자동형변환되므로
	사용 시 해당 자료형으로 캐스팅(형변환) 해야 한다. -->
	<ul>
		<li>아이디:
	...
	<!--  
		EL로 출력 시에는 별도의 형변환이 필요없고
		속성명이 중복되지 않는다면 xxScope계열의 내장객체를 생략할 수 있다.
	-->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!--  
	set태그의 target, property속성은 자바빈 객체의 속성
	혹은 컬렉션의 값을 설정할 때 사용하는 속성이다.
	target : 변수값을 지정하는 속성, 반드시 EL/표현식만 가능
	property : 값을 지정하는 속성, 표현식/EL/값 모두 가능
		- 자바빈인 경우 : 속성명(멤버변수)
		- Map컬렉션인 경우 : key값
	※ scope속성은 var속성이 지정된 태그에서만 지정 가능
	※ target과 propert를 사용해서 자바빈이나 컬렉션에 값을 설정할 때는
	var속성을 지정할 수 없다.
	-->
	...
	<!--  
		0번 인덱스로 저정된 defaultMember객체를 지정하여 정보를 출력한다.
	-->
	<h4>target 및 property속성으로 미설정 후 출력</h4>
	<ul>
	...
	<c:set target="${map.args }" property="name" value="초아짱" />
