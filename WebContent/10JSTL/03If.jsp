...
<body>
	...
	- else문이 없기 때문에 다중 조건일 때는 표현이 어렵다. 
	...
	<!--  
		if(numVar%2==0)와 동일한 조건으로 해당 조건문은 true를 반환하므로
	...
	<!--  
		JSTL의 if문은 else가 없기 때문에 참 or 거짓의 형태로
		구문을 만들 때는 아래와 같이 참의 조건, 거짓의 조건으로
		각각 블럭을 만들어 준다.
	-->
	<h3>문자열 비교</h3>
	...
	<c:if test="tRuE" var="result">
		TRUE(true) 대소문자 구분 없이 항상 참임
	</c:if>
	
	<h3>EL식으로 조건 판단 시 {}양쪽에 공백이 있으면
	무조건 false로 판단</h3>
	<c:if test="${true }"  var="result">
