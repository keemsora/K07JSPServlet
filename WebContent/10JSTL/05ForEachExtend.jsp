...
<body>
	<!--  
	forEach태그의 두 번째 문법 : 확장 for문
		주로 배열이나 컬렉션에서 값을 순차적으로 접근하고 싶을 때
		사용한다. varStatus에 지정된 값들 중 index속성은 0부터 시작한다.
	...
	- index : 현재 루프 내에서의 인덱스를 표시한다. 0부터 시작한다.
	...
	<ul>
		...
		<li>
			아이디:${maps.second.id },
			패스워드:${maps.second.pass },
			이름:${maps.second.name }
		</li>
		<li>
			아이디:${maps.third.id },
			패스워드:${maps.third.pass },
			이름:${maps.third.name }
		</li>
	</ul>
	
	<%--  
	루프 내에서 객체를 전달할 변수 map을 통해 맵의 key와 value를
	알아낼 수 있다.
		key => ${맵의이름.key}
		value => ${맵의이름.value}
	--%>
